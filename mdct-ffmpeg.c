/*
 * MDCT rotation transforms converted to integer precision
 * Copyright (c) 2010 Dave Hooper, Mohamed Tarek, Michael Giacomelli
 * Copyright (c) 2008 Loren Merritt (original floating point version)
 * Partly based on libdjbfft by D. J. Bernstein
 * Licensed under BSD for use in Tremor.
 */
 ...
 */
void ff_imdct_half(unsigned int nbits, ogg_int32_t *output, const ogg_int32_t *input) ICODE_ATTR_TREMOR_MDCT;
void ff_imdct_half(unsigned int nbits, ogg_int32_t *output, const ogg_int32_t *input)
{
    int n8, n4, n2, n, j;
    const ogg_int32_t *in1, *in2;
    ...
    /* permutation reorder the input and rotate;   result here is in OUTPUT ... */
    /* (note that when using the current split radix, the permuted ordering is
    ...
    /* Using the following pdf, you can see that it is possible to rearrange
       ...
       */
    const ogg_int32_t *T = sincos_lookup0;
    const int step = 2<<(12-nbits);
    const ogg_uint16_t * p_revtab=revtab;
    {
        const ogg_uint16_t * const p_revtab_end = p_revtab + n8;
    ...
    switch( nbits )
    {
        default:
        {
            ogg_int32_t * z1 = (ogg_int32_t *)(&z[0]);
            ogg_int32_t * z2 = (ogg_int32_t *)(&z[n4-1]);
        ...
        case 12: /* n=4096 */
        {
            ...
            const ogg_int32_t * V = sincos_lookup1;
            T = sincos_lookup0;
            ogg_int32_t t0,t1,v0,v1;
            ogg_int32_t * z1 = (ogg_int32_t *)(&z[0]);
            ogg_int32_t * z2 = (ogg_int32_t *)(&z[n4-1]);
 ...
 */
void ff_imdct_calc(unsigned int nbits, ogg_int32_t *output, const ogg_int32_t *input) ICODE_ATTR_TREMOR_MDCT;
#ifndef _ARM_ASSEM_
void ff_imdct_calc(unsigned int nbits, ogg_int32_t *output, const ogg_int32_t *input)
{
    const int n = (1<<nbits);
    const int n2 = (n>>1);
    const int n4 = (n>>2);
    ...
    ogg_int32_t * in_r, * in_r2, * out_r, * out_r2;
    ...
    while(in_r<in_r2)
    {
        register ogg_int32_t t0,t1,t2,t3;
        register ogg_int32_t s0,s1,s2,s3;
